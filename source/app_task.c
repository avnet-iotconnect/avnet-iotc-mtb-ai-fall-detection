...
static TickType_t previous_detection_ts = 0;

static int reporting_interval = 1500;
static int linger_interval = 4000; // linger fall detection for some time. Do it with the the LED as well.
...
void app_task(void *pvParameters) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; iotconnect_sdk_is_connected() && j < max_messages; j++) {
            bool detected =  was_fall_detected();
            TickType_t now = portTICK_PERIOD_MS * xTaskGetTickCount();
            if (detected) {
                fall_detected = true;
                previous_detection_ts = now;
            } else if (previous_detection_ts + linger_interval < now) {
                fall_detected = false; // expired
            }
            cyhal_gpio_write(CYBSP_USER_LED, fall_detected);
